...
export type UserType = {
    _id: string     //  to fix any
    name: string     //  to fix any
}

export const pureAddUserCallback = (name: string, setUsers: React.Dispatch<React.SetStateAction<UserType[]>>, users: UserType[]) => { //  to fix any
    const user = {
        _id: v1(), name // to fix
    }
    setUsers([...users, user])
...
const HW3 = () => {
    const [users, setUsers] = useState<Array<UserType>>([]) // to fix any

    const addUserCallback = (name: string) => { // to fix any
